"""Audit the E004 q=1/4 checkpoint without retraining or altering E004 artifacts."""
...
SOURCE_SCRIPT = Path("scripts/e004_q025_five_seed_replication.py")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="E004 q=1/4 checkpoint inclusion audit"
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        required=True,
        choices=[3, 4],
        help="E004 model seed to audit",
    )
    args = parser.parse_args()

    source_result = Path(
        f"results/e004/e004_k1_q025_seed{args.model_seed}_clean_attempt01.json"
    )
    audit_output = Path(
        f"results/e004/e004_k1_q025_seed{args.model_seed}_inclusion_audit_attempt01.json"
    )
    ...
    require(source_data["gate"] == "E004", "Source result is not E004")
    require(
    ...
    module.validate_output(
        ...
        "E004 checkpoint inclusion audit",
    )
    ...
    audit = {
        "audit": "E004 checkpoint clean-baseline inclusion audit",
    ...
    print("E004_INCLUSION_AUDIT: PASS")
