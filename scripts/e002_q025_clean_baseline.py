"""
E002 — PyGSD 1.2.0 q=1/4 Clean Baseline
...
def train_msgnn(shared: SharedData, seed: int) -> SeedResult:
    ...
    try:
        ...
        checkpoint_dir = Path("results/e002")
...
def main() -> int:
    """Run E002 clean baseline training."""

    # Parse arguments
    parser = argparse.ArgumentParser(
        description="E002 — PyGSD 1.2.0 q=1/4 Clean Baseline"
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        default=0,
        choices=[0],
        help="E002 model initialization seed; fixed to 0",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    expected_output_path = Path("results/e002/e002_k1_q025_seed0_clean_attempt01.json")
    ...
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error("E002 requires torch-geometric-signed-directed 1.2.0")
    # Print header
    print("=" * 60)
    print("E002 — PyGSD 1.2.0 q=1/4 Clean Baseline")
    print("=" * 60)
    ...
    if model_seeds != [0]:
        output_data["warnings"].append(
            "E002 requires exactly one model seed fixed to 0"
        )
