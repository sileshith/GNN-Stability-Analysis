"""E011 multi-seed primary-budget matched-retraining replication."""
...
CLEAN_RESULT_PATHS = {
    "q0125": {
        seed: Path(
            "results/e005/"
            f"e005_k1_q0125_seed{seed}_clean_attempt01.json"
        )
...
def checkpoint_path_for(scenario):
    require(MODEL_SEED is not None, "Model seed is not initialized")
    require(CHECKPOINT_DIR is not None, "Checkpoint directory is unset")
    return CHECKPOINT_DIR / (
        f"msgnn_k1_seed{MODEL_SEED}_"
        f"{scenario['name']}_attempt01.pt"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "E011 primary-budget matched-retraining replication"
        )
    )
    parser.add_argument(
        "--model-seed",
        ...
        required=True,
        choices=[1, 2, 3, 4],
        help="Model-initialization seed to replicate",
    )
    ...
    OUTPUT = Path(
        "results/e011/"
        f"e011_seed{MODEL_SEED}_"
        "primary_budget_matched_retraining_attempt01.json"
    )
    CHECKPOINT_DIR = Path("results/e011")
    CLEAN_RESULTS = {
        "q0125": {
            "q": 0.125,
            "path": CLEAN_RESULT_PATHS["q0125"][MODEL_SEED],
            "gate": "E005",
        },
        "q025": {
            "q": 0.25,
            "path": CLEAN_RESULT_PATHS["q025"][MODEL_SEED],
            "gate": "E003" if MODEL_SEED in {1, 2} else "E004",
        },
    }
    ...
    require(len(runs) == 5, "Unexpected retraining-run count")
    ...
    result = {
        "artifact": (
            "E011 multi-seed primary-budget matched-retraining "
            "replication"
        ),
        ...
        "limitations": [
            (
                "This artifact contains one model seed; cross-seed "
                "inference requires the later five-seed aggregate."
            ),
            (
                "Budget 58 is the sole primary intervention in this "
    ...
    print("=" * 70)
    print("E011_MULTI_SEED_MATCHED_RETRAINING: PASS")
    print("OUTPUT:", OUTPUT)
