"""
E001 — Clean Functional-Baseline Implementation
...
def train_msgnn(shared: SharedData, seed: int) -> SeedResult:
    ...
    try:
        ...
        checkpoint_dir = Path("results/e001")
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        checkpoint_name = (
            f"msgnn_k1_q0125_seed{seed}_"
...
def main() -> int:
    """Run E001 clean baseline training."""

    # Parse arguments
    parser = argparse.ArgumentParser(
        description="E001 — Clean Functional-Baseline Implementation"
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        default=0,
        choices=[0],
        help="E001 model initialization seed; fixed to 0",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    # Print header
    print("=" * 60)
    print("E001 — Clean Functional-Baseline Implementation")
    print("=" * 60)
    ...
    if model_seeds != [0]:
        output_data["warnings"].append(
            "E001 requires exactly one model seed fixed to 0"
        )
