"""
E004 — PyGSD 1.2.0 q=1/4 Model-Seed Replication
...
def train_msgnn(shared: SharedData, seed: int) -> SeedResult:
    ...
    try:
        ...
        checkpoint_dir = Path("results/e004")
...
def main() -> int:
    """Run E004 clean baseline training."""

    # Parse arguments
    parser = argparse.ArgumentParser(
        description="E004 — PyGSD 1.2.0 q=1/4 Model-Seed Replication"
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        required=True,
        choices=[3, 4],
        help="E004 model initialization seed; permitted values: 3 or 4",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    expected_output_path = Path(f"results/e004/e004_k1_q025_seed{args.model_seed}_clean_attempt01.json")
    ...
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error("E004 requires torch-geometric-signed-directed 1.2.0")
    # Print header
    print("=" * 60)
    print("E004 — PyGSD 1.2.0 q=1/4 Model-Seed Replication")
    print("=" * 60)
    ...
    # Add warnings
    if len(model_seeds) != 1 or model_seeds[0] not in {3, 4}:
        output_data["warnings"].append(
            "E004 requires exactly one model seed from {3, 4}"
        )
