"""
E005 — PyGSD 1.2.0 Fixed-Charge Comparison
...
installed PyGSD 1.2.0 package as a verified implementation reference following
...
class SeedResult:
    ...
    reload_max_abs_diff: float = float("inf")
...
def train_msgnn(shared: SharedData, seed: int) -> SeedResult:
    ...
    try:
        ...
        model.eval()
        with torch.no_grad():
            reference_val_output = msgnn_forward(
                shared.val_queries
            ).detach().clone()
        checkpoint_dir = Path("results/e005")
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        charge_tags = [
            tag
            for tag, value in CHARGE_VALUES.items()
            if value == config["q"]
        ]
        if len(charge_tags) != 1:
            raise RuntimeError(
                "Checkpoint charge does not map to exactly one condition"
            )
        charge_tag = charge_tags[0]
        checkpoint_name = (
            f"msgnn_k1_{charge_tag}_seed{seed}_"
            f"{start_time.strftime('%Y%m%dT%H%M%SZ')}.pt"
        )
        ...
        fresh_model = MSGNN_link_prediction(**config).to(DEVICE)
        ...
        result.reload_max_abs_diff = torch.max(
            torch.abs(val_output - reference_val_output)
        ).item()
        require(
            result.reload_max_abs_diff <= 1e-6,
            "Fresh-model checkpoint reload output mismatch",
        )
...
def main() -> int:
    """Run one locked E005 fixed-charge clean-baseline replicate."""

    # Parse arguments
    parser = argparse.ArgumentParser(
        description="E005 — PyGSD 1.2.0 Fixed-Charge Comparison"
    )
    parser.add_argument(
        "--charge",
        required=True,
        choices=sorted(CHARGE_VALUES),
        help="Fixed charge condition: q000=0 or q0125=0.125",
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        required=True,
        choices=[0, 1, 2, 3, 4],
        help="E005 model initialization seed; permitted values: 0 through 4",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    charge_value = CHARGE_VALUES[args.charge]
    MSGNN_CONFIG["q"] = charge_value
    output_path = Path(args.output)
    expected_output_path = Path(
        f"results/e005/e005_k1_{args.charge}_"
        f"seed{args.model_seed}_clean_attempt01.json"
    )
    if output_path != expected_output_path:
        parser.error(f"--output must be {expected_output_path}")
    if output_path.exists():
        parser.error(f"Refusing to overwrite existing output: {output_path}")
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error("E005 requires torch-geometric-signed-directed 1.2.0")
    # Print header
    print("=" * 60)
    print("E005 — PyGSD 1.2.0 Fixed-Charge Comparison")
    print("=" * 60)
    ...
    print(f"Charge condition: {args.charge}")
    print(f"Fixed q: {charge_value}")
    ...
    output_data = {
        ...
        "configuration": {
            "charge_condition": args.charge,
            "fixed_q": charge_value,
            "generation_seed": GENERATION_SEED,
            "split_seed": SPLIT_SEED,
        ...
        "results": [
            {
                ...
                "reload_max_abs_diff": (
                    None
                    if r.reload_max_abs_diff == float("inf")
                    else r.reload_max_abs_diff
                ),
    ...
    if (
    len(model_seeds) != 1
    or model_seeds[0] not in {0, 1, 2, 3, 4}
):
        output_data["warnings"].append(
            "E005 requires exactly one model seed from {0, 1, 2, 3, 4}"
        )
