"""Run the E006 fixed-checkpoint stratified perturbation pilot."""
...
MANIFEST = Path(
    "results/e006/"
    "e006_stratified_perturbation_manifest_attempt01.json"
)
MANIFEST_SHA256 = (
...
OUTPUT = Path(
    "results/e006/"
    "e006_fixed_checkpoint_stratified_pilot_attempt01.json"
)
...
BUDGETS = {
    "sign_reversal": {
        "signal_disrupting": [1, 2, 4, 8],
        "signal_restoring": [1],
    },
    "direction_reversal": {
        "signal_disrupting": [1, 2, 4],
        "signal_restoring": [1, 2, 4],
        "direction_neutral": [1, 2, 4],
    },
    "deletion": {
        "signal_removing": [1, 2, 4, 8],
        "noise_removing": [1, 2, 4],
    },
}
...
def scenario_result(
    ...
    return {
        ...
        "budget": budget,
        "selection_rule": (
            "First m records in frozen manifest lexicographic order"
        ),
        "selected_edges": [
            {
    ...
    for edit_type, stratum_budgets in BUDGETS.items():
        ...
        for stratum, budgets in stratum_budgets.items():
            ...
            for budget in budgets:
                ...
                selected = stratum_records[:budget]
                scenarios.append(
                    scenario_result(
                        module,
                        operator_module,
...
def main() -> int:
    ...
    module = load_module("e006_training_source", TRAINING_SOURCE)
    operator_module = load_module(
        "e006_operator_source",
        OPERATOR_SOURCE,
    )
    ...
    require(
        expected_scenarios_per_condition == 21,
        "Unexpected scenario-plan size",
    )
    require(
        all(
            condition["scenario_count"] == 21
    ...
    result = {
        "artifact": "E006 fixed-checkpoint stratified pilot",
        ...
        "limitations": [
            (
                "This is a deterministic seed-0 fixed-checkpoint pilot, "
                "not a random perturbation sample."
            ),
            (
                "The sign-restoring stratum has one eligible edge and "
                "is a case study, not replicated evidence."
            ),
            (
    ...
    print("E006_FIXED_CHECKPOINT_STRATIFIED_PILOT: PASS")
    print("OUTPUT:", OUTPUT)
