"""Audit one E005 fixed-charge checkpoint without retraining."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="E005 fixed-charge checkpoint inclusion audit"
    )
    parser.add_argument(
        "--charge",
        required=True,
        choices=sorted(CHARGE_VALUES),
        help="Fixed charge condition to audit",
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        required=True,
        choices=[0, 1, 2, 3, 4],
        help="E005 model seed to audit",
    )
    args = parser.parse_args()

    expected_q = CHARGE_VALUES[args.charge]
    source_result = Path(
    ...
    audit_output = Path(
        f"results/e005/e005_k1_{args.charge}_"
        f"seed{args.model_seed}_inclusion_audit_attempt01.json"
    )
    ...
    spec = importlib.util.spec_from_file_location(
        "e005_charge_checkpoint_audit_source",
        SOURCE_SCRIPT,
    )
    ...
    require(source_data["gate"] == "E005", "Source result is not E005")
    require(
    ...
    require(
        source_data["configuration"]["charge_condition"] == args.charge,
        "Source result charge-condition mismatch",
    )
    require(
        source_data["configuration"]["fixed_q"] == expected_q,
        "Source result fixed-q declaration mismatch",
    )
    ...
    module.validate_output(
        ...
        "E005 checkpoint inclusion audit",
    )
    ...
    audit = {
        "audit": "E005 checkpoint clean-baseline inclusion audit",
    ...
    print("E005_INCLUSION_AUDIT: PASS")
    print("CHARGE_CONDITION:", args.charge)
    print("FIXED_Q:", expected_q)
