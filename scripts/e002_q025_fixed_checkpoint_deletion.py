"""E002 fixed-checkpoint diagnostic: one eligible deletion.

This script does not train a model. It loads the accepted clean checkpoint,
freezes it, deletes exactly one eligible message-passing edge,
...
def load_clean_module():
    path = Path("scripts/e002_q025_clean_baseline.py")
    spec = importlib.util.spec_from_file_location("e002_clean", path)
...
def choose_eligible_edge(shared):
    ...
    absolute_degree = torch.zeros(
        shared.num_nodes,
        dtype=torch.float64,
    )
    half_absolute_weight = edge_weight.abs().to(torch.float64) / 2

    absolute_degree.index_add_(
        0,
        edge_index[0],
        half_absolute_weight,
    )
    absolute_degree.index_add_(
        0,
        edge_index[1],
    ...
    if not torch.all(absolute_degree > 0):
        raise RuntimeError(
            "Clean training graph contains a nonpositive absolute degree"
        )

    candidates = []
    for index, (source, target) in enumerate(pairs):
        weight = float(edge_weight[index])
        removed_degree = abs(weight) / 2
        ...
        if (source, target) in forbidden:
            continue
        if absolute_degree[source].item() - removed_degree <= 0:
            continue
        if absolute_degree[target].item() - removed_degree <= 0:
            continue

        candidates.append((source, target, index, weight))

    if not candidates:
        raise RuntimeError("No theorem-eligible deletion edge was found")

    candidates.sort(key=lambda item: (item[0], item[1], item[2]))
    return candidates[0], len(candidates)

def apply_deletion(edge_index, edge_weight, index):
    ...
    edited_index = edge_index[:, keep].clone()
    edited_weight = edge_weight[keep].clone()

    if edited_index.shape[1] != original_edge_count - 1:
        raise RuntimeError("Deletion did not remove exactly one edge")
    if edited_weight.shape[0] != edge_weight.shape[0] - 1:
        raise RuntimeError("Deletion did not remove exactly one weight")

    expected_index = torch.cat(
        (edge_index[:, :index], edge_index[:, index + 1:]),
        dim=1,
    )
    expected_weight = torch.cat(
        (edge_weight[:index], edge_weight[index + 1:]),
        dim=0,
    )

    if not torch.equal(edited_index, expected_index):
        raise RuntimeError("Deletion changed a remaining edge")
    if not torch.equal(edited_weight, expected_weight):
        raise RuntimeError("Deletion changed a remaining weight")
...
def absolute_degree(edge_index, edge_weight, num_nodes):
    degree = torch.zeros(num_nodes, dtype=torch.float64)
    half_absolute_weight = edge_weight.detach().cpu().abs().to(
        torch.float64
    ) / 2
    cpu_index = edge_index.detach().cpu()

    degree.index_add_(0, cpu_index[0], half_absolute_weight)
    degree.index_add_(0, cpu_index[1], half_absolute_weight)
...
def main():
    parser = argparse.ArgumentParser(
        description="E002 fixed-checkpoint deletion diagnostic"
    )
    parser.add_argument(
        "--clean-result",
        required=True,
        help="Accepted E002 clean-result JSON",
    )
    parser.add_argument(
        "--output",
        required=True,
    ...
    expected_clean_result = Path(
        "results/e002/e002_k1_q025_seed0_clean_attempt01.json"
    )
    expected_output = Path(
        "results/e002/"
        "e002_k1_q025_seed0_deletion_diagnostic_attempt01.json"
    )

    if clean_result_path != expected_clean_result:
        parser.error(f"--clean-result must be {expected_clean_result}")
    if output_path != expected_output:
        parser.error(f"--output must be {expected_output}")
    if output_path.exists():
        parser.error(f"Refusing to overwrite existing output: {output_path}")
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error(
            "Sign-reversal diagnostic requires PyGSD 1.2.0"
        )

    clean_record = json.loads(clean_result_path.read_text())
    if clean_record["gate"] != "E002":
        raise RuntimeError("Clean result is not labeled E002")
    ...
    perturbed_graph, perturbed_weights = apply_deletion(
        shared.train_graph,
        shared.train_weights,
        edge_index_position,
    )
    perturbed_weight = None
    ...
    expected_delta_h_fro = math.sqrt(0.5)

    if abs(delta_h_fro - expected_delta_h_fro) > 1e-9:
        raise RuntimeError("Theorem 5.3 deletion identity mismatch")

    clean_degree = absolute_degree(
        shared.train_graph,
        shared.train_weights,
        num_nodes,
    )
    perturbed_degree = absolute_degree(
        perturbed_graph,
        perturbed_weights,
        num_nodes,
    )
    clean_min_degree = float(clean_degree.min())
    perturbed_min_degree = float(perturbed_degree.min())
    d_star = min(clean_min_degree, perturbed_min_degree)

    degree_drop = clean_degree - perturbed_degree
    changed_degree_nodes = torch.nonzero(
        torch.abs(degree_drop) > ATOL,
        as_tuple=False,
    ).flatten().tolist()
    expected_degree_nodes = sorted([source, target])
    endpoint_degree_drop = abs(original_weight) / 2

    if sorted(changed_degree_nodes) != expected_degree_nodes:
        raise RuntimeError(
            "Deletion changed absolute degree outside the selected endpoints"
        )
    if abs(degree_drop[source].item() - endpoint_degree_drop) > ATOL:
        raise RuntimeError("Source absolute-degree change is incorrect")
    if abs(degree_drop[target].item() - endpoint_degree_drop) > ATOL:
        raise RuntimeError("Target absolute-degree change is incorrect")
    if d_star <= 0:
        raise RuntimeError(
            "Deletion violates the positive-degree theorem assumption"
        )
    ...
    result = {
        "gate": "E002-fixed-checkpoint-deletion-diagnostic",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "evidence_scope": (
            "One deterministic deletion using one frozen "
        ...
        "edit_manifest": {
            ...
            "selected_edge_removed": True,
            "remaining_edges_preserved": True,
            "exactly_one_edge_removed": True,
        ...
        "operator_diagnostics": {
            ...
            "clean_min_absolute_degree": clean_min_degree,
            "perturbed_min_absolute_degree": perturbed_min_degree,
            "d_star": d_star,
            "degree_changed_nodes": changed_degree_nodes,
            "expected_degree_changed_nodes": expected_degree_nodes,
            "endpoint_degree_drop": endpoint_degree_drop,
            "degree_changes_validated": True,
            "positive_degree_assumption_satisfied": d_star > 0,
        },
        ...
        "limitations": [
            ...
            "Theorem 5.3 deletion drift is charge-independent under the restricted unit-edge assumptions",
    ...
    print("E002 DELETION DIAGNOSTIC: PASS")
