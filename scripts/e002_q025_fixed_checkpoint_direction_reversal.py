"""E002 fixed-checkpoint diagnostic: one eligible direction reversal.

This script does not train a model. It loads the accepted clean checkpoint,
freezes it, reverses exactly one eligible message-passing edge,
...
def choose_eligible_edge(shared):
    ...
    if not candidates:
        raise RuntimeError("No eligible direction-reversal edge was found")

    candidates.sort(key=lambda item: (item[0], item[1], item[2]))
    return candidates[0], len(candidates)


def apply_direction_reversal(edge_index, edge_weight, index):
    edited_index = edge_index.clone()
    edited_weight = edge_weight.clone()

    source = int(edge_index[0, index].item())
    target = int(edge_index[1, index].item())

    if source == target:
        raise RuntimeError("Cannot reverse a self-loop")
    ...
    if (target, source) in set(original_pairs):
        raise RuntimeError("Direction reversal would create a reverse-edge collision")

    edited_index[0, index] = target
    edited_index[1, index] = source

    if not torch.equal(edited_weight, edge_weight):
        raise RuntimeError("Direction reversal changed an edge weight")

    changed_coordinates = torch.nonzero(
        edited_index != edge_index,
        as_tuple=False,
    )

    expected_coordinates = torch.tensor(
        [[0, index], [1, index]],
        dtype=changed_coordinates.dtype,
        device=changed_coordinates.device,
    )

    if not torch.equal(changed_coordinates, expected_coordinates):
        raise RuntimeError(
            "Direction reversal did not change exactly the selected endpoints"
        )

    edited_pairs = [
        (int(edited_index[0, i]), int(edited_index[1, i]))
        for i in range(edited_index.shape[1])
    ]

    if original_pairs.count((source, target)) != 1:
        raise RuntimeError("Original ordered edge was not unique")
    if edited_pairs.count((source, target)) != 0:
        raise RuntimeError("Original ordered edge remains after reversal")
    if edited_pairs.count((target, source)) != 1:
        raise RuntimeError("Reversed ordered edge was not created exactly once")
    if len(set(edited_pairs)) != len(set(original_pairs)):
        raise RuntimeError(
            "Direction reversal changed ordered-pair support cardinality"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="E002 fixed-checkpoint direction-reversal diagnostic"
    )
    parser.add_argument(
        "--clean-result",
        required=True,
    ...
    expected_output = Path(
        "results/e002/"
        "e002_k1_q025_seed0_direction_reversal_diagnostic_attempt01.json"
    )
    ...
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error(
            "Direction-reversal diagnostic requires PyGSD 1.2.0"
        )
    ...
    perturbed_graph, perturbed_weights = apply_direction_reversal(
        shared.train_graph,
    ...
    if abs(delta_h_fro - expected_delta_h_fro) > 1e-9:
        raise RuntimeError("Theorem 5.3 direction identity mismatch")
    ...
    result = {
        "gate": "E002-fixed-checkpoint-direction-reversal-diagnostic",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "evidence_scope": (
            "One deterministic direction reversal using one frozen "
        ...
        "edit_manifest": {
            ...
            "unordered_endpoints_preserved": True,
        ...
        "limitations": [
            ...
            "q=1/4 is the mentor-confirmed primary synthetic directed setting",
    ...
    print("E002 DIRECTION-REVERSAL DIAGNOSTIC: PASS")
