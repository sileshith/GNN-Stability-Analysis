"""Aggregate validated q=1/4 model-seed 0 through 4 evidence."""
...
OUTPUT = Path(
    "results/e004/"
    "e004_k1_q025_seeds0-4_five_run_aggregate_attempt01.json"
)
...
# Frozen before five-run aggregation. Strict inequality is required.
...
def main() -> int:
    ...
    require(
        ...
        "Run manifest must contain model seeds 0 through 4",
    )
        ...
        per_seed.append(
            {
                ...
                "sign_accuracy_gate_pass": (
                    matrix_accuracy(seed_sign_confusion)
                    > SIGN_ACCURACY_FLOOR
                ),
                "direction_accuracy_gate_pass": (
                    matrix_accuracy(seed_direction_confusion)
                    > DIRECTION_ACCURACY_FLOOR
                ),
            }
        )
    ...
    all_clean_runs_passed = all(
        run["class_behavior_pass"]
        for run in per_seed
    )
    all_accuracy_gates_passed = all(
        run["accuracy_gate_pass"]
        for run in per_seed
    )
    all_macro_f1_gates_passed = all(
        run["macro_f1_gate_pass"]
        for run in per_seed
    )
    all_sign_accuracy_gates_passed = all(
        run["sign_accuracy_gate_pass"]
        for run in per_seed
    )
    all_direction_accuracy_gates_passed = all(
        run["direction_accuracy_gate_pass"]
        for run in per_seed
    )
    full_clean_baseline_gate_passed = all(
        [
            all_clean_runs_passed,
            all_accuracy_gates_passed,
            all_macro_f1_gates_passed,
            all_sign_accuracy_gates_passed,
            all_direction_accuracy_gates_passed,
        ]
    )

    require(
        full_clean_baseline_gate_passed,
        "Frozen five-run clean-baseline gate failed",
    )

    aggregate = {
        "artifact": "E004 q=1/4 five-model-seed aggregate",
        ...
        "scope": {
            ...
            "interpretation": (
                "Five model-initialization/training replicates "
        ...
        "pooled_descriptive_counts": {
            "warning": (
                ...
                "they are descriptive and not 6,150 independent data cases"
            ),
        ...
        "gates": {
            ...
            "all_five_clean_runs_passed": all_clean_runs_passed,
            "all_five_class_behavior_audits_passed": (
                all_clean_runs_passed
            ),
            "all_five_accuracy_gates_passed": (
                all_accuracy_gates_passed
            ),
            "all_five_macro_f1_gates_passed": (
                all_macro_f1_gates_passed
            ),
            "all_five_sign_accuracy_gates_passed": (
                all_sign_accuracy_gates_passed
            ),
            "all_five_direction_accuracy_gates_passed": (
                all_direction_accuracy_gates_passed
            ),
            ...
            "full_clean_baseline_gate": (
                "PASS"
                if full_clean_baseline_gate_passed
                else "FAIL"
            ),
            "five_run_milestone": (
                "PASS"
        ...
        "scientific_interpretation": [
            (
                "The five runs are consistent across model "
                "initialization and training randomness."
            ),
            (
                "All five runs exceed the frozen joint-class "
                "majority baselines."
            ),
            (
                "All five runs predict every class and have "
                "nonzero recall for every class."
            ),
            (
                "All five runs learn direction substantially "
                "better than sign."
            ),
            (
            ...
            (
                ...
                "causal conclusion from these five runs."
            ),
        ],
        "next_milestones": [
            (
                "Run separately trained q=0 and q=1/8 "
                "synthetic charge controls"
            ),
            (
                "Create signal-related and signal-unrelated "
                "perturbation manifests with frozen budgets"
            ),
            (
    ...
    print("E004_FIVE_RUN_AGGREGATE: PASS")
    print("OUTPUT:", OUTPUT)
    print("MODEL_SEEDS:", EXPECTED_SEEDS)
    print("TOTAL_MODEL_RUNS: 5")
    print(
        "ACCURACY_MEAN:",
    ...
    print(
        "FIVE_RUN_MILESTONE:",
        aggregate["gates"]["five_run_milestone"],
    )
    print(
        "FULL_CLEAN_BASELINE_GATE:",
