"""
E003 — PyGSD 1.2.0 q=1/4 Model-Seed Replication
...
def train_msgnn(shared: SharedData, seed: int) -> SeedResult:
    ...
    try:
        ...
        checkpoint_dir = Path("results/e003")
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        checkpoint_name = (
            f"msgnn_k1_q025_seed{seed}_"
...
def main() -> int:
    """Run E003 clean baseline training."""

    # Parse arguments
    parser = argparse.ArgumentParser(
        description="E003 — PyGSD 1.2.0 q=1/4 Model-Seed Replication"
    )
    parser.add_argument(
        "--model-seed",
        type=int,
        required=True,
        choices=[1, 2, 3],
        help="E003 model initialization seed; permitted values: 1, 2, or 3",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    expected_output_path = Path(f"results/e003/e003_k1_q025_seed{args.model_seed}_clean_attempt01.json")
    ...
    if version("torch-geometric-signed-directed") != "1.2.0":
        parser.error("E003 requires torch-geometric-signed-directed 1.2.0")
    # Print header
    print("=" * 60)
    print("E003 — PyGSD 1.2.0 q=1/4 Model-Seed Replication")
    print("=" * 60)
    ...
    # Add warnings
    if len(model_seeds) != 1 or model_seeds[0] not in {1, 2, 3}:
        output_data["warnings"].append(
            "E003 requires exactly one model seed from {1, 2, 3}"
        )
